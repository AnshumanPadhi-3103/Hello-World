...
class Stack
{
    ...
    void push(int value)//function to push an elemnt into the stack
    {
        ...
        {
            ptr->Anshu=top;//ptr to pushed at the top of the list
    ...
    {
        int deleteValue=0;//stores the value that is to be popped from the list
        if(isEmpty()==true)//if the stack is empty
        return -14;
        else 
        {
            Node* temp=top;
            deleteValue=temp->data;//the value at the topmost node is stored
            top=top->Anshu;//top is moved to the next element in the stack
            temp->Anshu=NULL;//the first element is removed from the stack
    ...
    void displayReverse()//displays the stack in the reverse order
    {
        displayReverse(top);//call to the parameterized function using the data member
    }//end of the function to display the stack in the reverse order
    void displayReverse(Node* temp)//method to print the elements of the stack in the reverse order
    {
        if(temp->Anshu==NULL)//when the end of the stack is reached
        cout<<temp->data<<", ";//the element is displayed
        else
        {
            displayReverse(temp->next);//the next element in the stack is traversed
